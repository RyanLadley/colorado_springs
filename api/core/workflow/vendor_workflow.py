...
def new_vendor():
    ...
    print(vendor_form.get('new_materials'))
...
def update_vendor():
    ...
    return vendor_update.update_vendor_with_image(vendor) if vendor_form.get("image") else vendor_update.update_vendor(vendor)
...
@authorize()
def vendor_material(vendor_id):

    materials = materials_select.vendor_materials(vendor_id)

    return response.success(utilities.serialize_array(materials))